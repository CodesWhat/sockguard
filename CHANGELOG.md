...
- Corrected the roadmap and competitive-positioning docs to match the current code and current upstream comparators. The README, docs site, and internal vision doc now distinguish Sockguard's shipped structured access logs from the still-planned dedicated audit pipeline, give wollomatic proper credit for hostname/IP admission, per-container labels, optional bind-mount restrictions, JSON logging, and filtered unix-socket support, and expand the roadmap with the next real gaps: service/swarm inspection, broader read-side coverage, richer client identity, image trust policy, per-client abuse controls, and signed policy bundles / rollout modes.
...
- Added named per-client policy profiles. `clients.profiles` now defines reusable rulesets with profile-specific request-body inspection settings, `clients.source_ip_profiles` assigns them by caller CIDR, `clients.client_certificate_profiles` assigns them by verified mTLS client certificate common name, and `clients.default_profile` provides a fallback for unmatched callers. The root `rules` plus root `request_body` remain the fallback policy when no profile is selected. Profile rules participate in the same startup validation and blind-write guardrails as the root policy, access logs now record the selected profile, and `sockguard validate` prints configured profile sections.
- Added response filtering on successful Docker read calls. By default Sockguard now redacts `Config.Env` on `GET /containers/*/json`, redacts host-path details from container list/inspect mount metadata (`HostConfig.Binds` host sources and `Mounts[*].Source`), and redacts volume `Mountpoint` on `GET /volumes` plus `GET /volumes/*`. The proxy enforces this in `httputil.ReverseProxy.ModifyResponse` with bounded 8 MiB body buffering and fails closed with a generic `502` if a protected JSON response cannot be sanitized safely. Two explicit config/env opt-outs were added for trusted admin clients: `response.redact_container_env` / `SOCKGUARD_RESPONSE_REDACT_CONTAINER_ENV` and `response.redact_mount_paths` / `SOCKGUARD_RESPONSE_REDACT_MOUNT_PATHS`.
- Expanded read-side visibility control. `response.visible_resource_labels` and `clients.profiles[].response.visible_resource_labels` now inject Docker `label=` filters into container/image/network/volume list calls plus `GET /events`, hide inspect-style reads for non-matching resources behind `404`, and apply additively with selected client profiles. Sockguard also redacts container and network topology on container/network list and inspect responses by default via the new `response.redact_network_topology` / `SOCKGUARD_RESPONSE_REDACT_NETWORK_TOPOLOGY` toggle.
...
- Realigned the public-facing surfaces with the feature set that actually shipped in this Unreleased cycle. `README.md`'s Features table now credits request-body inspection on `/containers/create`, `/containers/*/exec`, `/images/create`, and `/build` instead of just `/containers/create`, the "Body-Blind Write Guardrail" row correctly narrows the remaining blind-write set to exec without an `allowed_commands` allowlist plus Swarm service writes, the Comparison table's "Per-client policies" and "Request body inspection" cells spell out the full sockguard feature, and the 0.4.0 roadmap line replaces the ambiguous "container/image pattern visibility" bullet with "name/image pattern visibility beyond label selectors" to match the still-future work described in `docs/content/docs/security.mdx`. `website/src/app/data/features.ts` gained new "Visibility-Controlled Reads" and "Named Client Profiles" cards and rewrote the Request Body Inspection card to cover all four endpoints; `website/src/app/data/comparison-rows.ts` flips "Response filtering" from Planned to shipped, expands the Request body inspection cell, and credits mTLS-cert profile routing under Per-client policies. `docs/content/docs/configuration.mdx` picked up a one-line note that unix listeners are hardened to `0600` owner-only and broader `listen.socket_mode` values are rejected at startup. Before this pass the website was still advertising a 0.2.0/0.3.0-era story even though the docs site had already caught up.
