...
- **README Quick Start now runs as `user: "0:0"`.** The sockguard image hardens to `USER 65534:65534` (nobody) by default, which is the right security posture but means the container can't read `/var/run/docker.sock` on a typical host where the socket is `root:docker`. The Quick Start's plaintext TCP example and the mTLS example both now explicitly set `user: "0:0"` (matching the unix-socket variant) with an inline comment explaining why and offering `user: "nobody:<docker-gid>"` as the hardened alternative. Tecnativa/LSIO work out-of-the-box because they run as root; sockguard's out-of-box example now matches that ergonomics without giving up the nobody default for users who mount the image some other way.
