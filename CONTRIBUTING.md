...
## Adding a config field (Go proxy)

New fields in `internal/config/config.go` need:

1. A `mapstructure` tag — this doubles as the YAML key and the `SOCKGUARD_<PATH>` env var name (dots become underscores, upper-cased).
2. A non-zero-value entry in `Defaults()`, if the default isn't the Go zero value.
3. Validation in `validate.go` if the field needs a rule.
4. A mapping in `ToFilterOptions()`/`filter.XxxOptions`/`newXxxPolicy()` if it feeds the request-body filter chain.

Viper default registration (`v.SetDefault(...)`) is generated by reflection off the `mapstructure` tags (`internal/config/load.go`'s `registerDefaults`) — adding a `v.SetDefault` line by hand is no longer a step. `internal/config/load_defaults_completeness_test.go` fails CI if a field is missing its `mapstructure` tag, or if `Load()` with no file/env input drifts from `Defaults()`.
