...
| **0.4.1** | Security hardening — host-network denial enforced regardless of opt-in flag, config preflight rejects explicit empty/missing `--config`, fuzz watchdogs replace `-timeout=0`, `scripts/local-fuzz.sh` gains `--timeout`/`--parallel`/`--suite ultra`, private vuln reporting enabled with published disclosure inboxes, and 100% Go test coverage restored | ✅ shipped |
