...
> [!NOTE]
> **v2.2.1 is the latest stable release.** It is a security patch on the v2.2 line: a `local` volume can no longer mount a raw host block device, a pseudo-filesystem like `proc` or `sysfs`, or an `overlay` lower/upper/work dir past `allowed_bind_mounts`, a path segment carrying a literal `*` can no longer slip past a deny rule whose segment glob is not a bare star, two byte-identical JSON keys in one object are rejected the same way case-variant pairs already were, and the visibility and owner-isolation filters can read a gzip-compressed upstream body instead of turning every scoped read into a `502`. The release also lifts the mutation score to 97.8%, pins the chart's image digest automatically after each stable publish, and moves the nightly deep-fuzz job off the harden-runner policy that was killing its legs. See [CHANGELOG.md](CHANGELOG.md) for the complete release notes.
