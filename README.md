...
| Tier | Theme |
|---|---|
| Security hardening (v1.x) | Continued mutation-test hardening of the rule-evaluation core and config validators (swarm `ContainerSpec` confinement parity and `SecurityOpt` `label=`/`systempaths=` evaluation shipped in v1.4; `CpuShares`-vs-hard-cap distinction for `require_cpu_limit` shipped in v1.5 as `require_cpu_limit_hard`; exec-`Env` allowlisting shipped in v1.5 as `allowed_env_vars`/`denied_env_vars`); audit-backlog follow-ups: non-`host` shared-namespace modes (`container:<id>`, `ns:<path>`) under `allow_host_network`, re-validate resource-limit requirements on container update (`allow_resource_updates` can silently strip a hard CPU/memory/PIDs cap set at create time), swarm-service CPU-limit parity (`request_body.service` has no `require_cpu_limit`/`require_cpu_limit_hard` equivalent today) |
