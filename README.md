...
> [!NOTE]
> **v1.4.2 is released.** The YAML schema, CLI flags, env vars, admin endpoints, and Prometheus metric names are stable under the v1.x contract. See [CHANGELOG.md](CHANGELOG.md) for the latest release notes and the current `Unreleased` work.
...
**v1.4.2 shipped on 2026-07-11** — a security patch over v1.4.1 backporting the case-varied-JSON-key filter-bypass fix from the v1.5 line (no config or API change) — and is the latest release. **v1.4.1 shipped on 2026-07-10** — a security patch over v1.4.0 (Go toolchain bumped to `1.26.5` for GO-2026-5856; no behavior change). **v1.4.0 shipped on 2026-07-10** with remote upstreams, confinement-mode parity, and supply-chain consolidation. **v1.0.0 shipped on 2026-05-20** with the YAML schema, CLI flags, env vars, admin endpoints, and Prometheus metric names under the v1.x compatibility contract. See [CHANGELOG.md](CHANGELOG.md) for the full per-release detail.
