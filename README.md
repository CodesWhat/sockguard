...
| **0.8.1** | Reload-UX patch surfaced by v0.8.0 NAS soak: (1) emit `reject_immutable` + leave `policy_version` unchanged when a reload would mutate an immutable field — currently the gate enforces immutability silently and logs `config reload applied`, so operators can think a change took effect when it didn't; (2) document and harden fsnotify file-watch on Synology / btrfs bind-mounts where inotify events from the host don't reliably propagate into the container — recommend `SIGHUP` as the canonical reload trigger on DSM, optionally watch the parent directory as a fallback; (3) full elimination of `metrics.Registry` scrape-lock contention between hot `observe` and cold `writePrometheus` (carried over from the v0.8.0 review sweep) | 🛠️ in progress |
