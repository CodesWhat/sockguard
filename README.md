...
> [!NOTE]
> **v2.1.0 is the latest stable release.** It promotes `2.1.0-rc.1` unchanged after the tri-tool conformance gate: native Podman `/libpod` write routes are now inspected on the same terms as their Docker-compat twins, owner isolation fails closed with `404`/`403`/`502` instead of forwarding an unresolved, foreign, or unverifiable target, and the libpod prune family and container commit are now owner-scoped. Container and pod process-list reads now require the read-exfiltration acknowledgment, and read-side redaction reaches libpod container inspect, volume, and network-topology responses. Some of these are breaking for deployments with owner isolation enabled; see [CHANGELOG.md](CHANGELOG.md) for the complete release notes.
