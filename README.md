...
| **0.7.0** | Abuse controls — per-client token-bucket rate limits, burst budgets, concurrency caps, per-endpoint cost weighting for expensive Docker operations, and a system-wide priority-aware fairness gate (`low`/`normal`/`high` profiles claim 50%/80%/100% of `clients.global_concurrency.max_inflight`) | 🛠️ in progress |
