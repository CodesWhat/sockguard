...
> [!NOTE]
> **v2.2.0 is the latest stable release.** It closes the volume-mount and read-side gaps that survived v2.1.0: the `local` volume driver's `type`/`o`/`device` options can no longer bind-mount a host path past `allowed_bind_mounts` on container create or on a Swarm service mount, `PUT /volumes/{name}` is inspected instead of being an unread write, a request target that is not a rooted path no longer reaches rule evaluation, and read-side redaction survives gzip encoding, `HEAD`, `304` revalidation and image inspect. It also adds `sockguard verify`, a configurable `server.shutdown_grace`, and a startup warning for a `SOCKGUARD_*` variable that matches no configuration key. One change is breaking: a `match.path` without a leading `/` now fails config validation instead of matching every rooted path. See [CHANGELOG.md](CHANGELOG.md) for the complete release notes.
