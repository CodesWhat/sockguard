...
func TestMediatorServeGRPCEndToEnd(t *testing.T) {
	...
	// Health/Check (Passthrough), not Solve: this test exercises end-to-end
	// h2c tunnel wiring, not Phase 3's Solve-specific per-message mediation
	...
	grpcResp, err := cc.RoundTrip(newGRPCRequest(t, "/grpc.health.v1.Health/Check", "payload"))
	if err != nil {
