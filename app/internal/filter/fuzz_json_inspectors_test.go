...
func FuzzVolume(f *testing.F) {
	...
	f.Add([]byte(`{"Driver":"local","DriverOpts":{"type":"none","o":"bind","device":"/"}}`))
	f.Add([]byte(`{"Driver":"local","DriverOpts":{"type":"ext4","device":"/dev/sda1"}}`))
	f.Add([]byte(`{"Driver":"local","DriverOpts":{"device":"//dev/sda1"}}`))
	f.Add([]byte(`{"Opts":{"type":"btrfs","o":"rw,device=/dev/sdb","device":"/dev/sda1"}}`))
	f.Add([]byte(`{"Driver":"local","DriverOpts":{"type":"tmpfs","o":"size=100m","device":"tmpfs"}}`))
	f.Add([]byte(`{`))
