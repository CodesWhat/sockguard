...
listener), or non-loopback plaintext TCP behind the two-flag insecure opt-in
(`admin.listen.insecure_allow_plain_tcp: true` plus
`admin.listen.insecure_allow_unauthenticated_clients: true` — one without the
other is rejected).
