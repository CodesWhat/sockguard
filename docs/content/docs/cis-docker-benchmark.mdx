...
| **5.5** | Sensitive host directories not mounted | `allowed_bind_mounts: []`: every host-path bind must be on the allowlist; default-empty rejects all host-path binds. Named and anonymous volumes aren't host paths and pass regardless |
