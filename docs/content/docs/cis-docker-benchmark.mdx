...
docker run -d \
  ...
  serve -c /etc/sockguard/cis-docker-benchmark.yaml
```

The preset is bundled in the container image at
`/etc/sockguard/cis-docker-benchmark.yaml`, so there's nothing to mount from
the host. Point `serve` at it with `-c`: the `--config` default is
`/etc/sockguard/sockguard.yaml` and there is no search path behind it, so a
config mounted under any other name is ignored and the container comes up on
the bundled deny-all default instead.

The preset carries no `listen:` block either, so on its own it listens on the
default `127.0.0.1:2375` and never creates a socket file.
`SOCKGUARD_LISTEN_SOCKET` is what puts it on the Unix socket the verification
steps below use.

Sockguard runs as UID 65532 inside the container, so replace
`<docker-socket-gid>` with the numeric group owner of the host socket
(`stat -c '%g' /var/run/docker.sock` on Linux, `stat -f '%g'` on macOS).
Without it the container can't open a stock `0660 root:docker` socket.
...
| **5.5** | Sensitive host directories not mounted | `allowed_bind_mounts: []`: every host-path bind must be on the allowlist; default-empty rejects all host-path binds. Named and anonymous volumes aren't host paths and pass regardless, except when their local-driver options request a bind, which is checked against the same allowlist |
...
| **5.11** | CPU priority set | `require_cpu_limit: true`: at least one of `NanoCpus`, `CpuQuota`, `CpuPeriod`, `CpuShares` must be set |
...
| **5.20** | Host UTS namespace not shared | Always on, no knob. `HostConfig.UTSMode: "host"` is rejected structurally |
...
| **5.24** | Cgroup usage confirmed | Always on, no knob. A non-empty `HostConfig.CgroupParent` is rejected structurally |
...
| **5.28** | PIDs cgroup limit used | `require_pids_limit: true`: `HostConfig.PidsLimit` must be a positive value; Docker's `-1` (unlimited) is non-zero but still rejected |
...
> **CPU priority vs. a hard cap.** CIS 5.11 only requires that *some* CPU
> constraint be set — `CpuShares` (a relative scheduling weight) satisfies
> it just as well as `NanoCpus` or `CpuQuota` (an absolute CFS budget). This
> preset intentionally leaves `require_cpu_limit_hard: false`: enabling it
> would reject `CpuShares`-only containers that are still fully CIS 5.11
> compliant. If your threat model cares about bounding *how much* CPU a
> container can consume — not just that a priority was set — add
> `require_cpu_limit_hard: true` on top of this preset; see
> [Configuration → Container-create body inspection](/configuration#container-create-body-inspection)
> for the distinction.
...
| **5.x runtime, uninspectable** | 5.6 SSH not running in container (image-level), 5.7/5.8 ports (network-level), 5.13 traffic binding, 5.14 restart policy, 5.18 ulimit override, 5.19 mount propagation, 5.26 healthcheck at runtime, 5.27 updated image, 5.29 default bridge | image scanner + dockerd flags + container-runtime audit |
...
1. **Negative test.** Try to run a non-compliant container through
   ...
   ```bash
   curl -X POST --unix-socket /var/run/sockguard/sockguard.sock \
