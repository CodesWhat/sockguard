...
| `service` | `allow_host_network`, `allowed_bind_mounts`, `allow_all_registries`, `allow_official`, `allowed_registries`, `allow_all_capabilities`, `allowed_capabilities`, `allow_sysctls`, `image_trust` | Denies host-network services and non-allowlisted bind mounts; allows Docker Hub official images by default (`allow_official: true`). Enforces the same `allowed_capabilities` / `allow_all_capabilities` capability gate, `allow_sysctls` sysctl gate, and `image_trust` cosign verification on the ContainerSpec image as container-create. |
...
> **Security note — label grants assume sockguard is the only socket consumer.** Allow labels are read from the calling container, and any workload that can reach the raw Docker socket directly can create a container carrying arbitrary `label_prefix` permission labels — self-granting access the policy never approved. Label ACLs are only trustworthy when every Docker API consumer goes through sockguard; the proxy cannot detect other socket consumers, so it logs a startup warning stating this invariant whenever `clients.container_labels.enabled` is set.
...
`ownership.*`, `insecure_allow_*`, and `upstream.request_timeout` (only
`upstream.socket` is pinned) — is rebuilt and atomically applied on every
successful reload.
