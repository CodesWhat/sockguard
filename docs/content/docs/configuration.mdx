...
- `upstream.endpoints[].insecure_skip_tls_verify` is deprecated in v2.1 and scheduled for removal in v3.0.0. Existing v2.1 configurations remain accepted and keep the same encrypted but unauthenticated wire behavior, with a startup warning naming `tls.ca_file` as the replacement. The Docker environment equivalent (`DOCKER_CERT_PATH` set while `DOCKER_TLS_VERIFY` is unset) has the same schedule and warns with `DOCKER_TLS_VERIFY=1` as its replacement. Configure the daemon's issuing CA or enable Docker TLS verification before v3.0.0.
