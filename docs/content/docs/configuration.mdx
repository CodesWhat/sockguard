...
- `POST /images/load` and native Podman's `POST /libpod/images/load` are inspected by default through the same policy. Sockguard denies image archive imports unless their `manifest.json` repo tags satisfy `request_body.image_load.*` registry policy, or untagged imports are explicitly allowed.
