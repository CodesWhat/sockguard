...
Sign the candidate and start Sockguard with both paths:

```bash
cosign sign-blob --bundle /etc/sockguard/sockguard.yaml.bundle /etc/sockguard/sockguard.yaml
...
- **Keyless (Fulcio + Rekor)**: `allowed_keyless` entries specify the exact
  ...
  transparency-log entry is also required. `verify_timeout` caps each network
  round trip.
