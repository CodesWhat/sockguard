...
- `insecure_accept_opaque_buildkit_tunnels` stays `false` by default and must be set explicitly before rules can admit `POST /session`, `POST /grpc`, or a `moby.buildkit.v1.Control` method path — the opaque BuildKit session/gRPC tunnel Compose uses for `docker compose build`/`up --build` by default (Engine API 1.53 deprecated both, but current Buildx still uses them). Unlike `POST /build`'s single inspectable JSON body, this transport carries secrets, SSH agent forwarding, and file sync as an ongoing binary conversation sockguard cannot bound once opened — a separate, dedicated ack rather than folding into `insecure_allow_body_blind_writes`. Tecnativa's `GRPC=1`/`SESSION=1` compat env vars auto-set this flag with a deprecation warning so existing drop-in configs keep working; see the [Compose/BuildKit Transport](/security#compose--buildkit-transport) section of the security model docs, and the `drydock-with-build.yaml`/`portwing-with-build.yaml` presets for the classic-builder (`DOCKER_BUILDKIT=0`) alternative that stays fully inspectable.
...
| `network` | `allow_custom_drivers`, `allow_swarm_scope`, `allow_ingress`, `allow_attachable`, `allow_config_only`, `allow_config_from`, `allow_custom_ipam_drivers`, `allow_custom_ipam_config`, `allow_ipam_options`, `allow_driver_options`, `allow_endpoint_config`, `allow_disconnect_force`, `allow_disable_ipv4` | Denies custom drivers, swarm/ingress/attachable/config-only controls, custom IPAM, driver options, forced disconnects, and (`allow_disable_ipv4`) an explicit `EnableIPv4: false` on network create. `allow_endpoint_config` gates endpoint static IP, MAC address, links, driver options, and `GwPriority` on **both** `POST /networks/*/connect` and `POST /containers/create`'s `NetworkingConfig.EndpointsConfig` (plus create's deprecated top-level `MacAddress` field) — one flag, two enforcement points. Endpoint `Aliases` are always allowed at both, regardless of this flag. |
...
| `libpod_secret` | *(reuses `secret`'s fields)* | Governs `POST /libpod/secrets/create` with the same `allow_custom_drivers`/`allow_template_drivers` semantics as the `secret` group above; libpod reads `driver` from the query string, so this inspector never reads the request body. |
...
| `SOCKGUARD_INSECURE_ACCEPT_OPAQUE_BUILDKIT_TUNNELS` | `insecure_accept_opaque_buildkit_tunnels` | `false`                 | Allow rules that match `POST /session`, `POST /grpc`, or a `moby.buildkit.v1.Control` method path — the opaque BuildKit session/gRPC tunnel Compose uses by default. Prefer the classic-builder (`DOCKER_BUILDKIT=0` + `POST /build`) presets when possible. |
...
| `SOCKGUARD_REQUEST_BODY_NETWORK_ALLOW_ENDPOINT_CONFIG`         | `request_body.network.allow_endpoint_config`            | `false` | Allow endpoint static IP/MAC/links/driver options/`GwPriority` on `POST /networks/*/connect` **and** `POST /containers/create`'s `NetworkingConfig.EndpointsConfig`. Aliases are always allowed regardless of this flag. |
...
| `SOCKGUARD_REQUEST_BODY_LIBPOD_VOLUME_ALLOW_DRIVER_OPTS`       | `request_body.libpod_volume.allow_driver_opts`          | `false` | Allow `POST /libpod/volumes/create` with a custom `Options` map. Reuses the `volume` group's fields under the libpod key. |
| `SOCKGUARD_REQUEST_BODY_LIBPOD_NETWORK_ALLOW_ENDPOINT_CONFIG`  | `request_body.libpod_network.allow_endpoint_config`     | `false` | Governs `POST /libpod/networks/create`. Reuses the `network` group's fields under the libpod key. |
| `SOCKGUARD_REQUEST_BODY_LIBPOD_SECRET_ALLOW_TEMPLATE_DRIVERS`  | `request_body.libpod_secret.allow_template_drivers`     | `false` | Allow `POST /libpod/secrets/create` with a template driver. Reuses the `secret` group's fields under the libpod key. |
