...
`global_concurrency`. Every client's consumed token-bucket quota and every
in-flight concurrency count reset to zero on the swap: a bucket that was
nearly drained comes back full, and a profile sitting at its `max_inflight`
cap opens back up. State is not carried across the old and new chains, and
there is no way to preserve it, since a profile can change shape between
reloads and there is no principled way to remap old counters onto a
different limit. Sockguard logs a warning when a reload replaces a chain
that had an active limiter, so this shows up as an expected reload side
effect rather than an unexplained gap in your throttling data.
