...
response:
  ...
  redact_host_topology: false           # redact GET /info host-fingerprinting fields (kernel/OS/arch/name/labels); opt-in
...
- `response.redact_host_topology` defaults to `false` (opt-in): when enabled, Sockguard redacts host-fingerprinting fields on `GET /info` — kernel version, OS, architecture, host name, and daemon labels — independent of Swarm mode (the redaction runs whether or not the daemon has a `Swarm` section in its response). Enable it when `/info` reads are exposed to a caller that should be able to confirm the daemon is reachable without learning host build details.
...
| `SOCKGUARD_RESPONSE_REDACT_HOST_TOPOLOGY`      | `response.redact_host_topology`       | `false`                         | Redact `GET /info` host-fingerprinting fields (kernel version, OS, architecture, host name, daemon labels). Opt-in.  |
...
- `redact_host_topology` (default `false`, opt-in): redacts `GET /info` host-fingerprinting fields — kernel version, OS, architecture, host name, and daemon labels — independent of whether the daemon is in Swarm mode.
