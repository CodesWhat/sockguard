...
| `SOCKGUARD_REQUEST_BODY_BUILDKIT_CONTROL_SOLVE_ALLOWED_EXEC_DIGESTS` | `request_body.buildkit.control.solve.allowed_exec_digests` | _empty_ | Comma-separated lowercase `sha256:` digests approving individual raw-LLB ExecOps while `build.allow_run_instructions` is false. Does not grant the Solve RPC or bypass execution guards. |
