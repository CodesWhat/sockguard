...
- `log.audit.enabled` (default `false`) enables a dedicated JSON audit stream with a stable schema: request ID, client request ID, raw and normalized path, decision, `reason_code`, reason, matched rule, selected profile, actor identity, transport identity, ownership context, and final HTTP status.
