...
| `network` | `allow_custom_drivers`, `allow_swarm_scope`, `allow_ingress`, `allow_attachable`, `allow_config_only`, `allow_config_from`, `allow_custom_ipam_drivers`, `allow_custom_ipam_config`, `allow_ipam_options`, `allow_driver_options`, `allow_endpoint_config`, `allow_disconnect_force` | Denies custom drivers, swarm/ingress/attachable/config-only controls, custom IPAM, driver options, and forced disconnects. `allow_endpoint_config` gates endpoint static IP, MAC address, links, and driver options on **both** `POST /networks/*/connect` and `POST /containers/create`'s `NetworkingConfig.EndpointsConfig` — one flag, two enforcement points. Endpoint `Aliases` are always allowed at both, regardless of this flag. |
