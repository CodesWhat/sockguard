...
| `exec` | `allow_privileged`, `allow_root_user`, `allowed_commands`, `allowed_env_vars`, `denied_env_vars` | Denies privileged/root exec and requires an argv allowlist before broad exec rules pass blind-write validation. `allowed_env_vars`/`denied_env_vars` optionally restrict exec-create `Env` entries by name only (create-time only; both empty by default = no restriction). |
