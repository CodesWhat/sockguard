...
- `POST /images/load` and native Podman's `POST /libpod/images/load` are inspected by default through the same policy. Docker archives are checked through `manifest.json` repo tags; OCI archives are checked through `index.json` `org.opencontainers.image.ref.name` annotations. On the native Podman route, bare names in either format are normalized to `localhost`, matching what Podman loads instead of Docker Hub semantics. Tagged references must satisfy `request_body.image_load.*` even when `allow_untagged` is enabled. Mixed Docker/OCI control files, malformed OCI metadata, and partial OCI layouts fail closed.
