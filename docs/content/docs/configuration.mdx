...
traffic through it, and every request it would have made comes back as a
`would_deny` audit event carrying the exact method and normalized path — the
answer to "what does this thing actually call" without reading its source or
guessing from vendor docs.

**1. Prerequisites.** The audit log is off by default (`log.audit.enabled`
defaults to `false` — see [Logging And Audit](#logging-and-audit)), and this
workflow reads nothing else, so it has to be on. The `discovery.yaml` preset
in `app/configs/` turns it on for you: a `discovery` profile in `mode:
audit` with a single deny-all rule, applied to every client via
`clients.default_profile`, with `log.audit.enabled: true` and the audit
stream on `stdout` (separate from the operational log, which stays on its
`stderr` default) so it's trivial to capture on its own.
...
Point the client at this sockguard instance exactly as it would run in
production — same socket path, same TLS/unix-peer setup — just with the
discovery preset in place of its real policy.
...
normal day. Nothing is blocked — `mode: audit` forwards every request to the
daemon regardless of the deny rule, so this is safe to run against a live
client, not just a sandbox. The only difference from running with no proxy
at all is that every would-be-denied request is now logged.
