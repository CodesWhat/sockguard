...
| `SOCKGUARD_REQUEST_BODY_VOLUME_ALLOW_DRIVER_OPTS`              | `request_body.volume.allow_driver_opts`                 | `false` | Allow `POST /volumes/create` with custom `DriverOpts`. Local-driver options that request a bind still have their `device` checked against `request_body.container_create.allowed_bind_mounts`. |
