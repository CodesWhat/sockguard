...
- `POST /images/create` is inspected by default. Sockguard blocks `fromSrc` imports unless `request_body.image_pull.allow_imports: true` and only allows Docker Hub official images unless you set `allow_all_registries: true` or list explicit `allowed_registries`. **Podman's native `POST /libpod/images/pull` shares the same `request_body.image_pull` block** — one registry allowlist governs both surfaces, so there is no separate `libpod_image_pull` group to configure and forget. libpod's query shape differs and is read on its own terms: the reference arrives in `reference`, not `fromImage`/`tag`, Podman matches that key case-insensitively and takes the last of a repeated pair, and it accepts a `docker://` transport prefix. Sockguard therefore folds the key, checks every repeated value rather than the first, and strips the transport prefix before matching the registry host. A libpod pull carrying no usable `reference` is denied rather than forwarded unless `allow_all_registries: true` (Podman rejects such a request itself). `allow_imports` does not apply to the libpod endpoint — libpod imports are a separate, currently uninspected route (`POST /libpod/images/import`).
