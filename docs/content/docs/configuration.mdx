...
- `metrics.enabled` is opt-in and serves Prometheus text metrics at `metrics.path` on the same listener. The endpoint is local to Sockguard, is never forwarded to Docker, bypasses Docker API allow rules like `/health`, and remains behind listener security plus `clients.allowed_cidrs`.
...
- `log.audit.enabled` (default `false`) enables a dedicated JSON audit stream with a stable schema: request ID, client request ID, raw and normalized path, decision, `reason_code`, reason, matched rule, selected profile, flattened actor and transport identity fields, ownership context, and final HTTP status.
...
SOCKGUARD_INSECURE_ALLOW_READ_EXFILTRATION=false
SOCKGUARD_UPSTREAM_SOCKET=/var/run/docker.sock
SOCKGUARD_LOG_LEVEL=debug
SOCKGUARD_LOG_FORMAT=json
SOCKGUARD_LOG_ACCESS_LOG=true
SOCKGUARD_LOG_AUDIT_ENABLED=true
SOCKGUARD_LOG_AUDIT_FORMAT=json
SOCKGUARD_LOG_AUDIT_OUTPUT=stderr
SOCKGUARD_METRICS_ENABLED=false
...
SOCKGUARD_RESPONSE_REDACT_SENSITIVE_DATA=true
...
SOCKGUARD_REQUEST_BODY_CONTAINER_CREATE_ALLOW_HOST_PID=false
SOCKGUARD_REQUEST_BODY_CONTAINER_CREATE_ALLOW_HOST_IPC=false
SOCKGUARD_REQUEST_BODY_CONTAINER_CREATE_ALLOWED_BIND_MOUNTS=/srv/containers,/var/lib/app-data
SOCKGUARD_REQUEST_BODY_CONTAINER_CREATE_ALLOW_ALL_DEVICES=false
SOCKGUARD_REQUEST_BODY_CONTAINER_CREATE_ALLOWED_DEVICES=/dev/dri,/dev/kvm
SOCKGUARD_REQUEST_BODY_CONTAINER_CREATE_ALLOW_DEVICE_REQUESTS=false
SOCKGUARD_REQUEST_BODY_CONTAINER_CREATE_ALLOW_DEVICE_CGROUP_RULES=false
...
SOCKGUARD_REQUEST_BODY_CONTAINER_UPDATE_ALLOW_RESTART_POLICY=false
SOCKGUARD_REQUEST_BODY_CONTAINER_ARCHIVE_ALLOWED_PATHS=/app/data,/tmp/import
SOCKGUARD_REQUEST_BODY_IMAGE_LOAD_ALLOW_UNTAGGED=false
SOCKGUARD_REQUEST_BODY_VOLUME_ALLOW_DRIVER_OPTS=false
SOCKGUARD_REQUEST_BODY_NETWORK_ALLOW_ENDPOINT_CONFIG=false
SOCKGUARD_REQUEST_BODY_SECRET_ALLOW_TEMPLATE_DRIVERS=false
SOCKGUARD_REQUEST_BODY_CONFIG_ALLOW_TEMPLATE_DRIVERS=false
SOCKGUARD_REQUEST_BODY_SERVICE_ALLOWED_BIND_MOUNTS=/srv/services
SOCKGUARD_REQUEST_BODY_SWARM_ALLOWED_JOIN_REMOTE_ADDRS=manager.internal:2377
SOCKGUARD_REQUEST_BODY_NODE_ALLOWED_LABEL_KEYS=com.sockguard.owner,team
SOCKGUARD_REQUEST_BODY_PLUGIN_ALLOWED_SET_ENV_PREFIXES=DEBUG=,LOG_LEVEL=
