...
- `POST /images/load` and native Podman's `POST /libpod/images/load` are inspected by default through the same policy. Docker archives are checked through `manifest.json` repo tags; OCI archives are checked through the effective `index.json` name, with `io.containerd.image.name` taking precedence over `org.opencontainers.image.ref.name` exactly as it does in Podman. On the native route, bare names are normalized to `localhost`. Tagged references must satisfy `request_body.image_load.*` even when `allow_untagged` is enabled, and every Docker manifest member is checked separately for an untagged image. Canonically duplicate control paths fail closed. Mixed archives follow Podman's OCI-first order and fall back to valid Docker metadata when the OCI controls are invalid.
