...
| `exec` | `allow_privileged`, `allow_root_user`, `allowed_commands`, `allowed_env_vars`, `denied_env_vars` | Denies privileged/root exec and requires an argv allowlist before broad exec rules pass blind-write validation — an empty `allowed_commands` denies every exec at request time too, unless the top-level `insecure_allow_body_blind_writes: true` is also set, in which case the argv-allowlist gate is lifted but `allow_privileged`/`allow_root_user`/`allowed_env_vars`/`denied_env_vars` still apply. `allowed_env_vars`/`denied_env_vars` optionally restrict exec-create `Env` entries by name only (create-time only; both empty by default = no restriction). |
