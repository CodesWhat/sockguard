...
group you may need `--group-add <docker-socket-gid>` or a matching `--user`
override. For Compose, add `group_add: ["<docker-socket-gid>"]` to the
Sockguard service. The hardening knobs that matter in practice are
