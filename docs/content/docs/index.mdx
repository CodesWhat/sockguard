...
> `SOCKGUARD_INSECURE_ALLOW_READ_EXFILTRATION=true` is only there to preserve broad `CONTAINERS=1` / `IMAGES=1` compatibility, including process-list, raw archive/export, and log/attach streaming endpoints. Remove it once you switch to tighter YAML list/inspect rules.
