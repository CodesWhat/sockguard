...
- **Per-client policies** — gate callers by source CIDR, mTLS client cert common name, or calling container labels, then apply named profiles
- **mTLS for remote TCP** — non-loopback TCP requires mutual TLS 1.3 by default, plaintext TCP is an explicit legacy opt-in
- **Structured access logging** — JSON logs with method, path, decision, matched rule, latency, request ID
