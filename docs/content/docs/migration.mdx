...
- A remote (git/HTTP) build context, or `-o type=local` / `type=tar` output: `request_body.build.allow_remote_context: true`, or `session.upload` / `session.file_send` respectively.
