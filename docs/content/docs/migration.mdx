...
The other v1.5 policy additions are opt-in: `restrict_namespace_sharing`, `allowed_namespace_sharing_containers`, `deny_namespace_path_mode`, `require_cpu_limit_hard`, and exec `allowed_env_vars`/`denied_env_vars`. See [Configuration](/configuration) for their exact semantics and defaults.
