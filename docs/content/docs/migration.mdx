...
The Docker environment fallback follows the same schedule. If `DOCKER_CERT_PATH` is set without `DOCKER_TLS_VERIFY`, add `DOCKER_TLS_VERIFY=1`; sockguard then verifies the daemon against `ca.pem` from that directory. A server-auth-only deployment without client certificates can set `DOCKER_TLS_VERIFY=1` with no `DOCKER_CERT_PATH` and use the host's system roots.
