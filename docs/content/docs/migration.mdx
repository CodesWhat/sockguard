...
The Docker environment fallback follows the same schedule. A non-empty `DOCKER_TLS` with `DOCKER_TLS_VERIFY` empty or unset selects unverified TLS; set `DOCKER_TLS_VERIFY=1` before v3.0.0. Any non-empty verification value, including `0` or `false`, enables verification and takes precedence over `DOCKER_TLS`, matching the Docker CLI's environment semantics. With `DOCKER_CERT_PATH` set, sockguard verifies against `ca.pem` from that directory and presents its `cert.pem` / `key.pem`; the path alone only locates TLS material and does not enable TLS. A server-auth-only deployment without client certificates can set `DOCKER_TLS_VERIFY=1` with no `DOCKER_CERT_PATH` and use the host's system roots.
