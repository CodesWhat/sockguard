...
|------------|-----------|
| `-frontend-addr` / `CETUSGUARD_FRONTEND_ADDR` | `listen.address` (TCP) or `listen.socket` (unix) — repeat the flag for multiple frontends and it maps to Sockguard's `listeners:` array (see [Multiple Listeners](/configuration#multiple-listeners)), not the singular `listen:` block |
