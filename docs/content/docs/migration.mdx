...
`11notes/docker-socket-proxy` is a fixed read-only proxy: it allows every Docker API `GET` except seven targeted exfiltration-prone endpoints and blocks every write. Its own regex for the image-get endpoint (`images/get(/|)$`) doesn't match the real `ImageGet`/`ImageGetAll` request shapes, so only six of the seven are reliably blocked ([11notes #12](https://github.com/11notes/docker-socket-proxy/issues/12)). There is no rule syntax to translate; the equivalent in Sockguard is a default-deny config that allows reads broadly while denying those same endpoints first (first-match-wins):

```yaml filename="sockguard.yaml"
rules:
  # Deny the seven exfiltration-prone reads 11notes targets
  # (11notes' own regex only reliably blocks six of these; see 11notes #12)
