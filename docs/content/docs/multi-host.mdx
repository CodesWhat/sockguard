...
- **`DOCKER_TLS_VERIFY` unset + `DOCKER_CERT_PATH` set** → encrypted, but the daemon's server certificate is *not* verified. This fallback remains compatible with the Docker CLI in v2.1, logs a source-specific deprecation warning, and will be removed in v3.0.0. Set `DOCKER_TLS_VERIFY=1` before upgrading.
