...
| `SOCKGUARD_UPSTREAM_FLAVOR` | `upstream.flavor` | `auto` | Which engine is behind the upstream: `auto`, `docker`, or `podman`. `auto` probes `GET /version` once at startup and fails startup if the answer is ambiguous; an explicit value never probes. Reload-immutable. See [Configuration](/configuration#yaml-config-recommended). |
