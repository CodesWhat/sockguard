...
info/events. It retains top for process monitoring, so the Docker-compatible
route requires `insecure_allow_read_exfiltration: true`: caller-selected
`ps_args` control the daemon host's `ps` output, whose process command lines
bypass response redaction. Every other exfiltration-gated endpoint (archive,
export, logs, attach, image/manifest push, `generate/kube`) stays excluded on
both surfaces. The preset allows no writes at all, so it never needs
