...
| `PUT /libpod/containers/{name}/archive` | `request_body.container_archive` — **shared with Docker-compat** | Podman routes this and Docker's `PUT /containers/{name}/archive` to a single `compat.Archive` handler, so both spellings carry the same required `path` query parameter and the same tar body. One matcher covers both, and `allowed_paths`, `allow_setid`, `allow_device_nodes` and `allow_escaping_links` apply identically on either. This is the only place a Docker-compat predicate deliberately fires on a libpod path, and it is safe precisely because there is no second wire format to read wrongly. |
| `POST /libpod/containers/{name}/update` | `request_body.container_update` — **shared with Docker-compat** | The same `allow_*` gates as the Docker-compat update, read against libpod's own shape rather than Docker's. The body is `UpdateEntities`, whose embedded OCI `specs.LinuxResources` flattens to root-level `memory`/`cpu`/`pids`/`blockIO`/`devices`/`unified` instead of Docker's PascalCase, there is no `HostConfig`/`Resources` nesting, and the restart policy arrives in the `restartPolicy`/`restartRetries` **query string** rather than the body — so `allow_restart_policy` is enforced against the query here, with the key folded and every repeated value checked, because Podman decodes it with gorilla/schema. `devices` rides `allow_all_devices`; the OCI resource keys, the per-device IO throttles (`DeviceReadBPs` and friends) and `r_limits` ride `allow_resource_updates`; `health_on_failure`, `no_healthcheck` and `health_startup_retries` ride `allow_restart_policy`, since all three configure what the daemon does to the container on its own initiative (Podman's own field doc calls the last one "the maximum number of retries before the startup healthcheck will restart the container"). `allow_privileged` and `allow_capabilities` are inert on this path: `UpdateEntities` has no field for either, so a container cannot be made privileged through it. Four fields have no gate at all and are refused outright — `health_cmd` and `health_startup_cmd` (a healthcheck command runs inside the container, so it is exec by another name; grant it through `request_body.exec` and the exec endpoints instead), `Env`/`UnsetEnv`, and `health_log_destination` (a directory on the daemon host). Healthcheck timing and log-rotation fields pass. |
...
pod and `?publishPorts` binds host ports. Nothing in sockguard reads that
archive, so an allow rule for the path requires
`insecure_allow_body_blind_writes: true`, exactly like `play/kube`.

## Read-Exfiltration Additions

`insecure_allow_read_exfiltration` is extended with libpod's equivalent of
every Docker-compat exfiltration surface, plus three libpod-only entries:
