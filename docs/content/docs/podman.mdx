...
`insecure_allow_read_exfiltration` covers libpod's native exfiltration
surfaces as well as the Docker-compatible routes:
...
this preset's narrow pod rules. Every other exfiltration-gated
endpoint (archive, export, logs, attach, image/manifest push, `generate/kube`)
stays excluded on both surfaces. The preset allows no writes at all, so it never needs
`insecure_allow_body_blind_writes`. See
