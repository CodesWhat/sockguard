...
| `POST /libpod/images/load` | `request_body.image_load` — **shared with Docker-compat** | Podman's native image load. Verified against Podman v5.8.1: the route takes no query parameters and the whole request body is the archive. Podman tries OCI archive before Docker archive, so Sockguard does too. Docker `manifest.json` `RepoTags` and OCI `index.json` effective names are checked against the same registry allowlist; `io.containerd.image.name` takes precedence over `org.opencontainers.image.ref.name`, matching Podman. Bare names are treated as `localhost/...`. Canonically duplicate control paths fail closed, valid mixed archives use OCI metadata first, invalid OCI metadata falls back to Docker metadata, and `allow_untagged` applies only to genuinely untagged entries. Not to be confused with `POST /libpod/local/images/load`, below. |
