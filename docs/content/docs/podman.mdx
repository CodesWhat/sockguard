...
| `POST /libpod/images/load` | `request_body.image_load` — **shared with Docker-compat** | Podman's native image load. Verified against Podman v5.8.1: the route takes no query parameters and the whole request body is the archive. Podman tries OCI archive before Docker archive, so Sockguard classifies the archive before admission: Docker `manifest.json` `RepoTags` and OCI `index.json` `org.opencontainers.image.ref.name` annotations are checked against the same registry allowlist. Bare names in either archive format are treated as `localhost/...`, matching the name Podman loads. Mixed Docker/OCI control files, malformed or partial OCI layouts, and tagged OCI references outside policy fail closed; `allow_untagged` applies only to genuinely untagged entries. Not to be confused with `POST /libpod/local/images/load`, below. |
