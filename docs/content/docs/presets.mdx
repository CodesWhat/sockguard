...
#         task list + inspect, GET /nodes/* (Swarm node inspect), events,
#         ping, version, info
...
Every resource path is an explicit list or inspect endpoint rather than a
...
Homarr's UI also exposes a `removeAll` container-delete action (also surfaced as
an MCP tool an LLM can drive), which this preset denies by default — the shipped
config only covers start/stop/restart. To opt in, add a delete rule ahead of the
catch-all deny:
...
```yaml
# Allows: GET container list, image inspect, GET /images/json, ping, version
...
Because `**` can match zero segments, the rule also allows the image metadata
list at `GET /images/json`; Diun does not call it, but it is not a
read-exfiltration endpoint.
