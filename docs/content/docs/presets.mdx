...
Optimized for the [Watchtower](https://github.com/nicholas-fedor/watchtower) container auto-updater.
...
`Name: "no"`. Moby serializes the embedded `Resources` fields at their zero
values in that request, which the daemon treats as no-ops; nonzero resource,
privilege, device and capability updates remain denied. Image list is
