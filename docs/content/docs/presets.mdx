...
# Response redactions are all off (redact_mount_paths, redact_container_env and
# redact_network_topology are false). Drydock recreates a container from its own
...
drydock presets it also disables all three response redactions, because
Watchtower recreates containers from the inspect payload it just read.
