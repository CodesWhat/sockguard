...
container process monitoring. Docker-compatible `GET /containers/{id}/top`
forwards caller-selected `ps_args` to the daemon host's `ps` and returns process
command lines without response redaction. Every other exfiltration-gated route
remains denied by the narrow rules. Point `upstream.socket` at either a rootful
