...
# (lookout's GetContainerLogs()); container logs can carry secrets. Drop the logs rule and this
...
# applies. insecure_allow_read_exfiltration=true is inherited from lookout.yaml for the
# GET /containers/*/logs rule (see the Lookout section above).
```
