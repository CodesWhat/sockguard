---
title: Presets
description: Ready-made sockguard configs for drydock, drydock with self-update, drydock with compose, drydock with build, Portwing, Portwing with exec, Portwing with compose, Portwing with build, Traefik, Portainer, Watchtower, Homepage, Homarr, Diun, Autoheal, GitHub Actions and GitLab runners, the CIS Docker Benchmark, read-only dashboards, and Podman read-only monitoring.
---
...
# instead, both denied here. Use drydock-with-build.yaml if you need `--build`
# support: it layers CLASSIC builder support (POST /build only, requires
# DOCKER_BUILDKIT=0 in the client) on top of this preset without opening
# /session or /grpc. See the Compose/BuildKit Transport section of the
# security model docs for the full breakdown.
...
# — BuildKit's opaque session/gRPC tunnel carries secrets, SSH agent
# forwarding, and file sync sockguard cannot inspect or bound once opened.
# THIS FAILURE IS INTENTIONAL: modern Buildx defaults to BuildKit, so a
# default `docker compose build` against this preset still fails with a
# /session or /grpc denial. Set DOCKER_BUILDKIT=0 in the client environment
# to fall back to the classic builder this preset actually supports.
# BuildKit gRPC mediation is a v1.7+ epic, not implemented here.
```
...
# with default settings never calls POST /build at all — it drives dockerd's
# embedded BuildKit over the hijacked, unversioned /session and /grpc streams
# instead, both denied here. Use portwing-with-build.yaml if you need
# `--build` support: it layers CLASSIC builder support (POST /build only,
# requires DOCKER_BUILDKIT=0 in the client) on top of this preset without
# opening /session or /grpc. See the Compose/BuildKit Transport section of
# the security model docs for the full breakdown.
...
```yaml
# Allows: GET list/inspect/stats/top/changes on containers, images, networks,
#         volumes, secrets — plus pod list/inspect (libpod only) and
#         health/version/info/events, on both surfaces
