...
runtimes. Container update allows restart-policy changes, which Watchtower uses
to set `Name: "no"`, while resource, privilege, device and capability updates
remain denied. Image list is explicitly denied before the registry-qualified
image-inspect rule. The preset keeps
