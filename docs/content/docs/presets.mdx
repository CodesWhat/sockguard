...
```yaml
# Allows: GET container list + inspect, network list + inspect, service and
#         task list + inspect (Swarm), events, ping, version, info
# Denies: everything else, including container archive/export/logs/attach
```

Every allowed path is a named list or inspect endpoint rather than a
...
```yaml
# Allows: GET container list + inspect + stats, image list, service list +
#         inspect and task list + inspect (Swarm), events, ping, version, info
...
`GET /images/{name}/history` do not end in `/json` and stay denied. That is
why this preset does **not** set `insecure_allow_read_exfiltration: true`.
