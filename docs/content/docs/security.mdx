...
Sockguard now applies the same honesty rule to every recognized data-exfiltration surface. Validation refuses to start broad rules that would expose `GET /containers/*/top`, `GET /libpod/containers/*/top`, `GET /libpod/pods/*/top`, `GET /containers/*/archive`, `GET /containers/*/export`, `GET /containers/*/logs`, `GET /containers/*/attach/ws`, `POST /containers/*/attach`, `GET /services/*/logs`, `GET /tasks/*/logs`, `GET /images/get`, `GET /images/*/get`, `POST /images/*/push`, or `POST /plugins/*/push` unless you explicitly set `insecure_allow_read_exfiltration: true`. Container and pod top return process command lines without response redaction; the Docker-compatible container route accepts caller-selected `ps_args` that run the daemon host's `ps`, while both native Podman top routes accept caller-selected `ps_args` and can stream repeated process tables. All three normalized process-list routes pass through one hard request-time acknowledgment gate too, so an exact-name rule or an ordered deny that shadows validation's representative path cannot bypass the requirement. Image and plugin pushes are Docker API writes, but they read local artifacts and transmit them to a caller-selected registry, so they carry the same exfiltration risk; the option name is retained for configuration compatibility. Because startup validation only sees method + path, `/containers/*/logs` is treated conservatively whether or not the caller also sets `follow=1`. That keeps process monitoring, backup/export, raw-stream, and deliberate registry-publish use cases possible without letting a casual wildcard rule silently include them.
