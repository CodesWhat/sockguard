...
not admit an unlabeled batch member. A foreign, unlabeled, missing, or
lookup-failed member stops the whole request, and the original query is
forwarded byte-for-byte only after all checks pass. Values are percent-decoded
once and repeated keys form the list. Commas remain part of one value because
Podman's string-slice decoder does not split them. Podman's `References` and
`Images` key spellings are case-insensitive; Sockguard combines every casing in
arrival order, deduplicates decoded identifiers for ownership lookup, and
refuses more than 256 selected values before deduplication so one action cannot
amplify into unbounded daemon inspect work.
...
store's narrower behavior. An omitted `names` list is still forwarded to the
daemon's own no-reference error.
