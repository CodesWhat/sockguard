...
| Native gRPC-over-h2c | `moby.buildkit.v1.Control/*` | No — same control plane, bare HTTP/1.1 transport | Denied unconditionally — sockguard's listener has no h2c support outside the two hijack-capable tunnel endpoints, so nothing can dial this path end-to-end |
...
must prove that resource exists before the request can continue. In `enforce`
mode a not-found resource returns `403` instead of being forwarded to the
daemon, because a missing inspect result establishes no owner. A profile in
`warn` or `audit` mode logs the would-be denial and forwards the request under
the existing rollout contract. The same rule applies when an exec session
cannot be resolved to its container and when a `container:<ref>`
namespace-sharing target is missing. `allow_unowned_images` applies only to an
image that resolves successfully and has no owner label. Paths that do not
identify a resource still pass through this layer, while an inspect error
remains a `502` lookup failure.
...
the active label and name/image-pattern axes. A foreign, unlabeled, missing,
hidden, or lookup-failed member stops the whole request, and the original query
...
is still forwarded to the daemon's own no-reference error.
...
`lookupManifest=false` when owner-scoped image removal is required.
...
- Refuse `GET /libpod/system/df` and `GET /libpod/containers/showmounted` with a `403`, since neither native response carries labels for any policy axis to read (see Layer 6 above)
...
`denied_env_vars`, and `allowed_env_values` are enforced on `POST /containers/*/exec`, not on the
