...
`request_body.buildkit.control.solve` gates the `Solve` RPC: `security.insecure` is always denied with no enabling knob, `network.host` requires `request_body.build.allow_host_network`, and cache import/export types, cache registries, exporter types, and exporter-push registries each have their own allowlist (empty = deny, the standard `request_body.*` convention). The frontend check pairs with `request_body.build.allow_run_instructions`: a Dockerfile synced over the session's `FileSync` stream gets the identical RUN-instruction hold-and-inspect scan the classic `/build` path already applies, unless it arrives via a genuinely remote context, which requires `request_body.build.allow_remote_context` for the same reason classic `/build` does — both flags are the sibling `request_body.build` block's own fields, reused verbatim rather than duplicated under `buildkit`. `control.allow_status` gates the `Status` RPC, admitted only for a ref this same client/profile actually `Solve`d, so one tenant cannot poll another's build. The session RPCs buildkitd calls back into the client for — `session.auth` (registry credentials, exact realm/scope match), `session.secrets` and `session.ssh` (exact-ID allowlists), `session.file_sync`/`file_send` (byte/file/path caps), and `session.upload` (a one-use token bound to an admitted `Solve`) — each stay denied until their own `allow`/allowlist fields are set. See [Configuration](/configuration) for the full field reference.
