...
`POST /images/create` and native Podman's `POST /libpod/images/pull` are inspected by default through the same `request_body.image_pull` policy. Sockguard blocks `fromSrc` imports unless explicitly allowed and constrains pulls to Docker Hub official images unless the operator opts into `allow_all_registries` or an explicit registry allowlist. The libpod endpoint carries its reference in a `reference` query parameter rather than Docker's `fromImage`, and Podman matches that key case-insensitively and honors the last of a repeated pair, so Sockguard folds the key and checks every value, strips a leading `docker://` transport before matching the host, and denies a libpod pull that carries no usable reference whenever a registry allowlist posture is in force. Native `POST /libpod/images/load` and `POST /libpod/images/import` are separate, currently uninspected endpoints and are not covered by `allow_imports`; allow rules that reach either one require `insecure_allow_body_blind_writes: true`.
...
Image batch export and removal get a stricter preflight because forwarding one
mixed batch can disclose or delete several images before a later member fails.
In `enforce` mode, Sockguard resolves every repeated `names`, `references`, or
`images` query value on `GET /images/get`,
`GET /libpod/images/export`, and `DELETE /libpod/images/remove` before the
daemon sees the action. Every member must carry the caller's owner label;
`allow_unowned_images` does not admit an unlabeled batch member. A foreign,
unlabeled, missing, or lookup-failed member stops the whole request, and the
original query is forwarded byte-for-byte only after all checks pass. Values
are percent-decoded once and repeated keys form the list. Commas remain part
of one value because neither Moby nor Podman's string-slice decoder splits
them. Podman's `References` and `Images` key spellings are case-insensitive;
Sockguard combines every casing in arrival order, deduplicates decoded
identifiers for ownership lookup, and refuses more than 256 selected values
before deduplication so one action cannot amplify into unbounded daemon
inspect work.

Two collection-expanding forms are refused instead of guessed at. Podman's
batch remove treats an empty `images` list as a request to enumerate and remove
the matching image collection, even when `all=false`, so owner isolation
requires at least one image. With a non-empty `images` list, Podman takes the
named-image branch even when the last `all` value is true, so that form remains
available after each named image passes preflight. Moby treats a name-only
`names=team/app` export as every tagged image in that repository, so the
Docker-compatible batch route requires an explicit tag, digest, or full image
ID. Podman's native export resolves a name-only reference to one image and can
still use it. These are preflight checks, not a daemon transaction: an image
tag can still be rebound between inspect and action, and a later daemon-side
operational failure can still leave Podman's own multi-image remove partially
applied. The ownership gate guarantees that no initially unauthorized batch is
forwarded; it cannot make the upstream API atomic.
...
- Refuse `GET /libpod/system/df` with a `403`, since Podman's native disk-usage report carries no labels for any policy axis to read (see Layer 6 above)
