...
When `metrics.enabled` is true, Sockguard serves Prometheus text metrics from `/metrics` by default. The scrape endpoint is local to Sockguard, is never forwarded to Docker, bypasses Docker API allow rules like `/health`, and remains behind listener security plus client ACLs.
