...
Sockguard now applies the same honesty rule to raw archive/export reads. Validation refuses to start broad read rules that would expose `GET /containers/*/archive`, `GET /containers/*/export`, `GET /images/get`, or `GET /images/*/get` unless you explicitly set `insecure_allow_read_exfiltration: true`. That keeps backup/export use cases possible without letting a casual `GET /containers/**` or Tecnativa-style section gate silently include filesystem tar streams.
...
When `ownership.owner` is set, Sockguard stamps label-capable creates and build-produced images with an owner label, injects `label=` filters into list/prune/events responses, and inspects target resources on individual requests to deny cross-owner access. That now covers owned containers, images, networks, volumes, services, tasks, secrets, and configs, with service writes stamping both the service and its task template so downstream tasks inherit the same owner identity. This turns one shared Docker socket into N isolated identity views.
...
- Fail startup unless raw archive/export reads are explicitly acknowledged via `insecure_allow_read_exfiltration: true`
