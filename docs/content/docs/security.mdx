...
must prove that resource exists before the request can continue. A not-found
resource returns `403` instead of being forwarded to the daemon, because a
missing inspect result establishes no owner. The same rule applies when an exec
session cannot be resolved to its container and when a `container:<ref>`
