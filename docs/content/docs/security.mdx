...
Modern `docker build` and `docker compose build` no longer speak the classic `POST /build` API by default — Buildx routes through BuildKit's opaque, unversioned session/gRPC tunnel (`POST /session` for the frontend/session bridge, `POST /grpc` for the `moby.buildkit.v1.Control` gRPC service, both tunneled over a hijacked HTTP/1.1 connection). Sockguard cannot bound or inspect either stream once it opens: unlike `POST /build`'s bounded, inspectable tar build-context body, the session/gRPC transport carries secrets, SSH agent forwarding, and live file sync as an ongoing binary conversation. Supported transports, by preset:

| Transport | Endpoint(s) | Inspectable | Default posture |
|-----------|-------------|--------------|------------------|
| Classic builder | `POST /build` | Yes — remote context, host network, `RUN` instructions | Allowed when `request_body.build` is configured (see the `*-with-build.yaml` presets) |
| BuildKit session/gRPC | `POST /session`, `POST /grpc` | No — opaque hijacked stream | Denied; requires `insecure_accept_opaque_buildkit_tunnels: true` |
| Native gRPC-over-h2c | `moby.buildkit.v1.Control/*` | No — same opaque control plane, different transport | Denied; also requires the same ack, though sockguard's listener has no h2c support today so nothing can dial it end-to-end yet |

To keep BuildKit-based Compose builds working without silently admitting an uninspectable tunnel, set `DOCKER_BUILDKIT=0` (classic builder) on the client and use the `drydock-with-build.yaml` / `portwing-with-build.yaml` presets, which layer a scoped `POST /build` allow rule with `request_body.build` inspection on top of the respective `-with-compose.yaml` baseline. This is a deliberate, documented gap, not an oversight — if you see a build fail against the default presets with a `/session` or `/grpc` denial, **this failure is intentional**: either switch the client to the classic builder, or explicitly acknowledge the risk with `insecure_accept_opaque_buildkit_tunnels: true` (a dedicated flag, separate from `insecure_allow_body_blind_writes`, because the tunnel isn't a bounded-body write sockguard merely declines to inspect — it's a stream sockguard cannot see into at all). Tecnativa's `GRPC=1` / `SESSION=1` compat env vars keep working unchanged and auto-set this acknowledgment with a deprecation warning nudging toward the explicit key. Full BuildKit gRPC mediation — parsing and enforcing policy on the `Solve` request within the tunnel rather than gating the tunnel wholesale — is tracked as a v1.7 epic.
