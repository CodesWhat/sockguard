...
`POST /images/load` and native `POST /libpod/images/load` are inspected by default. Docker `manifest.json` repo tags and OCI `index.json` ref-name annotations are checked against the same official/registry allowlist model used for pulls. Bare names in either archive format follow Podman's `localhost` normalization on the native route. Mixed-format or malformed OCI archives fail closed, and `allow_untagged` never bypasses a tagged OCI reference.
