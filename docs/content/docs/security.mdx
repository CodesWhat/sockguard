...
Sockguard now applies the same honesty rule to every recognized data-exfiltration surface. Validation refuses to start broad rules that would expose `GET /containers/*/top`, `GET /containers/*/archive`, `GET /containers/*/export`, `GET /containers/*/logs`, `GET /containers/*/attach/ws`, `POST /containers/*/attach`, `GET /services/*/logs`, `GET /tasks/*/logs`, `GET /images/get`, `GET /images/*/get`, `POST /images/*/push`, or `POST /plugins/*/push` unless you explicitly set `insecure_allow_read_exfiltration: true`. Container top accepts caller-selected `ps_args`, runs the daemon host's `ps`, and returns process command lines without response redaction; versioned `/vN.N/containers/{id}/top` requests normalize to the same route. Image and plugin pushes are Docker API writes, but they read local artifacts and transmit them to a caller-selected registry, so they carry the same exfiltration risk; the option name is retained for configuration compatibility. Because this validation layer only sees method + path, `/containers/*/logs` is treated conservatively whether or not the caller also sets `follow=1`. That keeps process monitoring, backup/export, raw-stream, and deliberate registry-publish use cases possible without letting a casual wildcard rule silently include them.
