...
Future body-inspection additions on the roadmap: deeper secure-create rails such as escalation capabilities like `SYS_ADMIN`/`NET_ADMIN`, no-new-privilege / non-root rails, resource defaults, and security profile overrides.
...
Future read-side visibility / redaction additions still tracked for the next tier:
