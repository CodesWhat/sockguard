...
Each inspector reads its request body under a per-endpoint byte cap and returns `413 Payload Too Large` when the cap is exceeded, instead of streaming unbounded bodies into memory. A malformed or hostile client cannot tie up the filter or the Docker daemon with oversized payloads — the bounded reader short-circuits before the JSON decode or tar parse even begins.
...
When `log.audit.enabled` is true, Sockguard also emits a dedicated JSON audit event with a stable schema: request ID, client request ID, raw and normalized path, decision, machine-readable `reason_code`, human-readable reason, matched rule, selected profile, actor identity, transport identity, ownership context, and final HTTP status. Upstream reverse-proxy errors overwrite the audit reason code with bounded values such as `upstream_socket_unreachable` or `upstream_response_rejected_by_policy`, so the terminal result remains explicit even after an allow decision has already been made.

Prometheus metrics remain planned operator-observability work, but dedicated audit events and explicit deny reason codes are current runtime behavior.
