...
- `response.redact_container_env` and `response.redact_mount_paths` default to `true`. Sockguard redacts `Config.Env` on `GET /containers/*/json`, redacts container `HostConfig.Binds` host paths plus `Mounts[*].Source` on `GET /containers/json` and inspect responses, and redacts volume `Mountpoint` on `GET /volumes` and `GET /volumes/*`. Disable them only for trusted admin clients that truly need Docker's raw metadata.
...
`response.redact_container_env` and `response.redact_mount_paths` control response filtering on successful Docker read calls:
