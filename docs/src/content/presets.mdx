...
# Container-create bodies are inspected, but exec/build/swarm writes are still method+path only
# so this preset opts into insecure_allow_body_blind_writes=true until those layers ship
```
...
# Container-create bodies are inspected; exec bodies are still method+path only so this
# preset opts into insecure_allow_body_blind_writes=true for exec until that layer ships
```
