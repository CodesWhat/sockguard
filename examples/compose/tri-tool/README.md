...
| Exec denied | Yes |
| Denial response verbosity | `deny_verbosity: verbose` — denial bodies include the request method/path and the detailed rule/exec-policy reason (e.g. `not allowed by portwing preset`), not just the generic message; safe here because the only consumer is the trusted Portwing agent / drydock controller, not an end user |
