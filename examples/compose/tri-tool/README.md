...
drydock should log `Edge agent connected: portwing-edge-<id> (version=…, drydockCompat=…)` once Portwing dials in — Edge mode does not emit Standard mode's `Handshake successful` line, and the connected name is a generated `portwing-edge-<id>`, not your `AGENT_NAME`. If it doesn't connect, check **Portwing's** logs, not drydock's — drydock rejects a bad hello silently (an error frame to the client, nothing server-side): `controller rejected hello: Unknown or revoked key (unknown-key)` (or `bad-signature`) means `portwing_authorized_keys` doesn't contain the key Portwing is presenting (re-run `portwing keygen -pub-from portwing_ed25519.pem` and re-append its output, or register the key live with `POST /api/v1/portwing/keys` instead); `ECONNREFUSED` from Portwing means drydock isn't up yet. On drydock's side, a startup `Failed to load DD_PORTWING_AUTHORIZED_KEYS` warning means the keys file is world-readable or unreadable by its node user (UID 1000) — see the chown/chmod above.
