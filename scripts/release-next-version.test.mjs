...
describe("inferReleaseLevel", () => {
  ...
  it("returns patch for other patch types", () => {
    for (const type of ["style", "perf", "test", "build", "ci", "chore", "revert"]) {
      assert.equal(
  ...
  it("returns minor when feat mixed with patch types", () => {
    assert.equal(inferReleaseLevel(["🐛 fix: a fix", "✨ feat: a feature"]), "minor");
  });
  ...
  it("returns major for bang after scope", () => {
    assert.equal(inferReleaseLevel(["✨ feat(api)!: breaking change"]), "major");
  });
  ...
  it("returns major immediately even if other commits follow", () => {
    assert.equal(
      inferReleaseLevel(["🐛 fix: minor fix", "✨ feat!: breaking", "✨ feat: another feature"]),
      "major",
    );
  });

  it("ignores commits without conventional prefix", () => {
    assert.equal(inferReleaseLevel(["merge branch main", "🐛 fix: actual fix"]), "patch");
  });
...
describe("release-next-version CLI", () => {
  it("prints a manual bump result", () => {
    const result = spawnSync(
      process.execPath,
      [scriptPath, "ignored", "--current", "1.2.3", "--bump", "minor"],
      {
        encoding: "utf8",
        env: cleanEnv,
      },
  ...
  it("computes an auto bump from git history", () => {
    ...
    const result = spawnSync(
      process.execPath,
      [scriptPath, "--current", "1.2.3", "--from", fromRef],
      {
        cwd: repoDir,
        encoding: "utf8",
        env: cleanEnv,
  ...
  it("computes an auto bump with an explicit to ref", () => {
    ...
    const result = spawnSync(
      process.execPath,
      [scriptPath, "--current", "1.2.3", "--from", fromRef, "--to", toRef],
      {
        cwd: repoDir,
        encoding: "utf8",
        env: cleanEnv,
