...
import { DOG_ART } from "./sockguard-dog-art";
...
// The serve-banner dog is the real 24-bit truecolor half-block art from
// app/internal/banner/dog_color.ans, rendered cell-by-cell from DOG_ART below
// (generated by scripts/gen-dog-art.mjs) so the demo matches exactly what
// `sockguard serve` prints on a truecolor terminal.
                ...
                if (line.kind === "colorart") {
                  return (
                    <pre
                      key={line.id}
                      aria-hidden="true"
                      className="mx-auto w-fit whitespace-pre font-mono text-[0.6rem] leading-[0.6rem] tracking-normal"
                    >
                      {DOG_ART.map((row) => (
                        <div key={row.id}>
                          {row.runs.map((run) => (
                            <span
                              key={run.id}
                              style={{
                                color: run.f ?? undefined,
                                backgroundColor: run.b ?? undefined,
                              }}
                            >
                              {run.c.repeat(run.n)}
                            </span>
